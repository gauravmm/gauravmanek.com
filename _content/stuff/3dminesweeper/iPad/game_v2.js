...
function game_singleClick(x, y, z){
	...
	if((gameBoard[z][y][x] & 2) == 0)
		game_floodfillReveal(x, y, z);
	
	if(clearCount + numMinesTotal == totalVolume)
		game_handleVictory();
}
function game_altClick(x, y, z, flagTile){
	if(freezeStats)
		return;
	
	if((gameBoard[z][y][x] & 2) == 2)
	...
	if(flagTile === undefined){
		...
		gameBoard[z][y][x] = gameBoard[z][y][x] ^ 4;
	} else {
		if((gameBoard[z][y][x] & 4) != 4 && flagTile) {
			++flagCount;
			gameBoard[z][y][x] = gameBoard[z][y][x] ^ 4;
		} else if((gameBoard[z][y][x] & 4) == 4 && !flagTile){
			--flagCount;
			gameBoard[z][y][x] = gameBoard[z][y][x] ^ 4;
		}
	}
